...
- `apply.sh` runs under `set -euo pipefail`. Steps that can legitimately fail on a fresh machine (such as App Store auth) must degrade gracefully — warn and continue — rather than abort the run, matching the existing `brew bundle` pattern. — *Enforced by: human review*
...
- `apply.sh` must pass `make lint` (shellcheck) before commit. — *Enforced by: `make lint` (shellcheck); no CI configured — run locally*
